...
<div class="max-w-6xl px-8 pt-6 pb-8 mx-4 bg-white rounded-lg shadow-md sm:container sm:mx-auto sm:w-full">
  @if ($image->title != null)
      <h3 class="mb-4 text-4xl">{{ $image->title }}</h3>
  @endif
  @ownsImage($image)
    <form role="form" method="POST" action="{{ route('image.infos', ['image' => $image->name]) }}">
      ...
      <div class="my-6 sm:flex sm:items-center">
        ...
        <button class="px-4 py-2 mt-4 font-bold text-white bg-indigo-700 rounded shadow hover:bg-indigo-800 focus:shadow-outline focus:outline-none sm:mx-4 sm:mt-0" type="submit">
          <i class="fas fa-save"></i> Save
        </button>
        <a href="#" class="px-4 py-2 mt-4 font-bold text-white bg-red-700 rounded shadow hover:bg-red-800 focus:shadow-outline focus:outline-none sm:mx-4 sm:mt-0" onclick="deleteImage()">
  ...
  <div class="sm:flex sm:flex-row">
    ...
    <div class="my-6 sm:ml-16 sm:my-0">
      <h3 class="pb-3 text-2xl font-medium text-gray-900 font-firacode">Image Tools</h3>
      <a href="{{ route('image.download', ['image' => $image->name]) }}">
        <button class="block px-4 py-2 mb-4 font-semibold text-gray-900 bg-transparent border rounded hover:bg-gray-600 hover:text-white w-36 border-grey hover:border-transparent">
          <i class="fa fa-download"></i> Download
        </button>
      </a>
      <button class="block px-4 py-2 my-4 font-semibold text-gray-900 bg-transparent border rounded modal-open-embed hover:bg-gray-600 hover:text-white w-36 border-grey hover:border-transparent">
        <i class="fas fa-code"></i> Embed
      </button>
      <button class="block px-4 py-2 my-4 font-semibold text-gray-900 bg-transparent border rounded hover:bg-gray-600 hover:text-white w-36 border-grey hover:border-transparent">
