...
@section('head')
<title>{{ $user->username }}'s Albums — {{ config('app.name', 'Laravel') }}</title>

<!-- OpenGraph/Twitter -->
<meta data-rh="true" name="description" content="{{ $user->username }}'s Albums" />
<meta data-rh="true" property="og:url" content="{{ url()->current() }}" />
<meta data-rh="true" property="og:description" content="{{ $user->username }}'s Albums" />
...
<meta data-rh="true" property="twitter:description" content="{{ $user->username }}'s Albums" />
